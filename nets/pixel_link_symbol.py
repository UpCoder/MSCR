...
class PixelLinkNet_multiphase_multislice_mask_multiscale(object):
    '''
    相比于PixelLinkNet_multiphase_multislice_mask，我们针对segmentation的branch使用multiscale的方法来做，
    注意，我们也没有使用GCLSTM去提取特征，而是直接concat
    '''
    ...
    def _fuse_by_cascade_conv1x1_upsample_sum(self, num_classes, scope, multiscale_flag=False):
        """
        ...
        import config
        num_layers = len(config.feat_layers)
        feature_map_in_diff_resolution = []
        with tf.variable_scope(scope):
            smaller_score_map = None
            for idx in range(0, len(config.feat_layers))[::-1]:  # [4, 3, 2, 1, 0]
                current_layer_name = config.feat_layers[idx]
                ...
                feature_map_in_diff_resolution.append(smaller_score_map)
        if not multiscale_flag:
            return smaller_score_map
        else:
            return smaller_score_map, feature_map_in_diff_resolution
    ...
    def _fuse_feat_layers(self):
        ...
        if config.feat_fuse_type == FUSE_TYPE_cascade_conv1x1_upsample_sum:
            ...
            with tf.variable_scope('pixel_seg'):
                pixel_seg_base, pixel_seg_base_multiscale = self._fuse_by_cascade_conv1x1_upsample_sum(
                    32, scope='extract_feature', multiscale_flag=True)
                pixel_seg_logits = pixel_seg_base
                _, h, w, c = pixel_seg_base.get_shape().as_list()
                for pixel_seg_base_singlescale in pixel_seg_base_multiscale[:-1]:
                    pixel_seg_base_singlescale = tf.image.resize_images(pixel_seg_base_singlescale, [h, w])
                    pixel_seg_logits = tf.concat([pixel_seg_logits, pixel_seg_base_singlescale], axis=-1)
                self.pixel_seg_logits = self._score_layer(pixel_seg_logits, config.num_tumor_type,
                                                          scope='logits_layer')
    ...
    def build_loss(self, pixel_cls_labels, pixel_cls_weights,
        ...
        with tf.name_scope('pixel_link_loss'):
            ...
            _, h, w, c = self.pixel_seg_logits.get_shape().as_list()
            resized_original_input = tf.image.resize_images(self.mask_input, [h, w],
                                                            tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            print('the mask input is ', self.mask_input)
            print('the resized original_input is ', resized_original_input)
            # seg_loss, dice_tensor, dice_tensors = compute_seg_loss(tf.squeeze(self.mask_input), original_seg_logists, num_classes=5)
            seg_loss = compute_seg_loss(tf.squeeze(resized_original_input, axis=-1), self.pixel_seg_logits,
                                        num_classes=5)
...
class PixelLinkNet_multiphase_multislice_clstm_mask_multiscale(object):
    '''
    相比于PixelLinkNet_multiphase_multislice_clstm_mask，我们针对segmentation的branch使用multiscale的方法来做
    '''
    def __init__(self, nc_inputs, art_inputs, pv_inputs, mask_input, is_training, batch_size_ph):
        self.nc_inputs = nc_inputs
        self.batch_size_ph = batch_size_ph
...
class PixelLinkNet_multiphase_multislice_clstm(object):
    def __init__(self, nc_inputs, art_inputs, pv_inputs, is_training, batch_size_ph, multiscale_flag):
        self.nc_inputs = nc_inputs
        self.batch_size_ph = batch_size_ph
        self.art_inputs = art_inputs
        self.pv_inputs = pv_inputs
    ...
    def _fuse_feat_layers(self):
        ...
        if config.feat_fuse_type == FUSE_TYPE_cascade_conv1x1_upsample_sum:
            if self.multiscale_flag:
                print('execute clstm with multiscale without mask')
                pixel_cls_base, pixel_cls_base_multiscale = self._fuse_by_cascade_conv1x1_upsample_sum(
                    16, scope='multiscale_pixel_cls', multiscale_flag=True)
                pixel_cls_logits = pixel_cls_base
                _, h, w, c = pixel_cls_base.get_shape().as_list()
                for pixel_cls_base_singlescale in pixel_cls_base_multiscale[:-1]:
                    pixel_cls_base_singlescale = tf.image.resize_images(pixel_cls_base_singlescale, [h, w])
                    pixel_cls_logits = tf.concat([pixel_cls_logits, pixel_cls_base_singlescale], axis=-1)
                self.pixel_cls_logits = self._score_layer(pixel_cls_logits, config.num_classes,
                                                          scope='multiscale_cls_layer')

                pixel_link_base, pixel_link_base_multiscale= self._fuse_by_cascade_conv1x1_upsample_sum(
                    32, scope='multiscale_pixel_link', multiscale_flag=True)
                pixel_link_logits = pixel_link_base
                _, h, w, c = pixel_link_base.get_shape().as_list()
                for pixel_link_base_singlescale in pixel_link_base_multiscale[:-1]:
                    pixel_link_base_singlescale = tf.image.resize_images(pixel_link_base_singlescale, [h, w])
                    pixel_link_logits = tf.concat([pixel_link_logits, pixel_link_base_singlescale], axis=-1)
                self.pixel_link_logits = self._score_layer(pixel_link_logits, config.num_neighbours * 2,
                                                          scope='multiscale_link_layer')
...
class PixelLinkNet_multiphase_multislice(object):
    def __init__(self, nc_inputs, art_inputs, pv_inputs, is_training, multiscale_flag):
        self.nc_inputs = nc_inputs
        self.art_inputs = art_inputs
        self.pv_inputs = pv_inputs
        self.multiscale_flag = multiscale_flag

...
def compute_seg_loss(gt_tensor, pred_tensor, num_classes, epsilon=1e-6, axis=[1, 2]):
    from nets.lovasz_losses_tf import lovasz_with_softmax
    ...
    # use the lovasz loss
    # base_ce_loss = tf.losses.sparse_softmax_cross_entropy(logits=pred_tensor, labels=gt_tensor, reduction='none',
    #                                                       loss_collection=None)
    # print('the cross_entropy loss is ', base_ce_loss)
    # print('the mask is ', gt_tensor)
    # ce_loss_base = base_ce_loss * tf.cast(
    #     tf.greater_equal(gt_tensor, 1), tf.float32)
    # print('the ce_loss_base is ', ce_loss_base)
    #
    # cross_entropy_loss_tensor = lovasz_with_softmax(pred_tensor, gt_tensor,
    #                                                 ce_loss_base)
    # print('we use the lovasz loss')



    # only use the original cross entropy
    cross_entropy_loss_tensor = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(logits=pred_tensor,
                                                                                      # weights=weights,
                                                                                      loss_collection=None,
                                                                                      labels=gt_tensor))
    print('we use the original cross entropy')

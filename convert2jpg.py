...
def extract_bboxs_mask_from_mask(mask_image, tumor_types):
    ...
    with open(tumor_types, 'r') as f:
        ...
        for i in range(1, maximum + 1):
            ...
            res_mask += filled_mask # segmentation的结果不存在overlap的现象
...
def statics_benign_malignant(dataset_dir):
    total_benign = 0
    total_malignant = 0
    for sub_name in ['train_gt', 'val_gt', 'test_gt']:
        benign = 0
        malignant = 0
        folder_path = os.path.join(dataset_dir, sub_name)
        for name in os.listdir(folder_path):
            txt_path = os.path.join(folder_path, name)
            with open(txt_path, 'r') as txt_file:
                lines = txt_file.readlines()
                type_names = [line.split(' ')[0] for line in lines]
                for type_name in type_names:
                    if type_name in ['CYST', 'FNH', 'HEM']:
                        benign += 1
                        total_benign += 1
                    elif type_name in ['HCC', 'METS']:
                        malignant += 1
                        total_malignant += 1
                    else:
                        print type_name
                        assert False
        print(sub_name, 'the number of benign is ', benign)
        print(sub_name, 'the number of malignant is ', malignant)
    print('Total the number of benign is ', total_benign)
    print('Total the number of malignant is ', total_malignant)
...
if __name__ == '__main__':
    ...
    # # 针对多个期相
    # image_dir = '/home/give/Documents/dataset/LiverLesionDetection_Splited/1'
    # LiverLesionDetection_Iterator(
    #     image_dir,
    #     dicom2jpg_multiphase_tripleslice_mask,
    #     '/home/give/Documents/dataset/LiverLesionDetection_Splited/JPG/1',
    #     ['NC', 'ART', 'PV'],
    #     'PV'
    # )

    statics_benign_malignant('/home/give/Documents/dataset/LiverLesionDetection_Splited/JPG/1/NCARTPV_tripleslice_mask')
